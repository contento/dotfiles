...
The directory module shows the full path under `$HOME` rather than collapsing to the repo root (`truncate_to_repo = false`) and uses `home_symbol = "…"` so a path under home reads `…/Projects/...` — clearly under home, not an absolute root path.
